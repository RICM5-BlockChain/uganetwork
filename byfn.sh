...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the ugachain
...
# and two Peer Orgs (``Polytech`` & ``IAE``) each managing and maintaining two peer nodes.
...
# (``peer0.polytech.ugachain.com`` & ``peer0.iae.ugachain.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for PolytechMSP   ##########"
  echo "#################################################################"
  ...
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for PolytechMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for IAEMSP   ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/IAEMSPanchors.tx -channelID $CHANNEL_NAME -asOrg IAEMSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for IAEMSP..."
    exit 1
  fi
  echo
}
